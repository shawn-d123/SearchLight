...
`description` and `rationale` are plain English and **surface on screen** during the simulation state — see §6. `weight` comes from the family prior, not from the model.
...
## 5. Trajectory batch — worker to orchestrator
...
```json
{
  "bounds": {"north": 32.62, "south": 32.24, "east": -110.52, "west": -111.00},
  "resolution": 256,
