...
Usage:  python pipeline/validate_fixtures.py     (exit 0 = clean, 1 = violations)
"""
...
def main():
    ...
    ext = json.load(open(FIXTURES / "extraction.json"))
    ...
    tr = (FIXTURES / "transcript.txt").read_text(encoding="utf-8")
    ...
    # --- trajectories -------------------------------------------------------
    batches = json.load(open(FIXTURES / "trajectories.json"))
    ...
    for n in ("field.json", "field_partial.json", "field_collapsed.json"):
        fields[n] = json.load(open(FIXTURES / n))
    ...
    # --- fleet status -------------------------------------------------------
    frames = json.load(open(FIXTURES / "fleet_status.json"))
