...
Usage:  python pipeline/make_fixtures.py [--runs-per-batch N]

Writes: fixtures/case.json
        fixtures/trajectories.json
        fixtures/field.json
        fixtures/field_partial.json
        fixtures/field_collapsed.json
        fixtures/fleet_status.json
"""
...
DATA, FIXTURES = ROOT / "data", ROOT / "fixtures"
...
def main():
    ...
    if args.stress:
        ...
        for d in (FIXTURES, ROOT / "web" / "public" / "fixtures"):
            d.mkdir(parents=True, exist_ok=True)
            (d / "trajectories_12k.json").write_text(text, encoding="utf-8")
        print("  wrote trajectories_12k.json  {:.1f} MB  ({} runs)".format(
            (FIXTURES / "trajectories_12k.json").stat().st_size / 1e6, n_runs))
        print("  standard mocks left untouched. Point FIXTURES.trajectories at")
        print("  /fixtures/trajectories_12k.json in lib/config.ts to stress test.")
        return
    ...
    public = ROOT / "web" / "public" / "fixtures"
    ...
    for name, obj in out.items():
        ...
        for d in (FIXTURES, public):
            (d / name).write_text(text, encoding="utf-8")
        print("  wrote {:<22} {:>8.1f} KB  (fixtures/ and web/public/fixtures/)"
              .format(name, (FIXTURES / name).stat().st_size / 1024))

    # Intake fallback: a key types this at speaking pace if the mic fails.
    for d in (FIXTURES, public):
        (d / "transcript.txt").write_text(TRANSCRIPT, encoding="utf-8")
    print("  wrote {:<22} {:>8.1f} KB  (fixtures/ and web/public/fixtures/)"
          .format("transcript.txt", (FIXTURES / "transcript.txt").stat().st_size / 1024))

    # bbox.json is imported directly by web/lib/config.ts.
    (ROOT / "web" / "lib" / "bbox.json").write_text(
