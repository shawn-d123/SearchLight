...
export function createFixtureSource(): Source {
  ...
  async function load(): Promise<Loaded> {
    ...
    loading = (async () => {
      ...
      const batches =
        stress ?? (await fetchJson<TrajectoryBatch[]>(FIXTURES.trajectories)) ?? [];
      ...
      const [
        ...
        fetchJson<unknown>(FIXTURES.case),
        fetchJson<unknown>(FIXTURES.simStarted),
        fetchJson<unknown[]>(FIXTURES.fleetStatus),
        fetchJson<FieldUpdate>(FIXTURES.fieldPartial),
        fetchJson<FieldUpdate>(FIXTURES.field),
        fetchJson<FieldUpdate>(FIXTURES.fieldCollapsed),
        fetchJson<unknown>(FIXTURES.validation),
        fetchText(FIXTURES.transcript),
        fetchJson<Record<string, unknown>>(FIXTURES.extraction),
      ]);
