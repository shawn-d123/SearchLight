...
export const FIXTURES = {
  ...
  trajectories: "/fixtures/trajectories.json",
  /**
   * 12,000 runs at 60 points each, for the frame-rate check. Gitignored —
   * regenerate with `python pipeline/make_fixtures.py --stress`. The loader falls back
   * to `trajectories` when it is absent, so a clean clone still runs.
   */
  trajectories12k: "/fixtures/trajectories_12k.json",
  field: "/fixtures/field.json",
  fieldPartial: "/fixtures/field_partial.json",
  fieldCollapsed: "/fixtures/field_collapsed.json",
  fleetStatus: "/fixtures/fleet_status.json",
  transcript: "/fixtures/transcript.txt",
  extraction: "/fixtures/extraction.json",
  /** Generated by scripts/make-web-fixtures.py — see that file's header. */
  simStarted: "/fixtures/sim_started.json",
  validation: "/fixtures/validation_result.json",
} as const;
 ...
 * and it is tuned against real trajectories — `pipeline/tune_evidence.py` runs the
 ...
 * settings.
 *
 * Measured on a real 12,000-run pipeline (pipeline/tune_evidence.py), radius 3,250 m:
 *
